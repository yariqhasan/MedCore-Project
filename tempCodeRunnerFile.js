...
// Fetch data from appointment table
 ...
 app.get("/PaymentApproved", (req, res) =>{
    ...
    connection.query('SELECT `processed_payment`.`confirm_code`,`amount`,`processed_payment`.`billing_name`,`processed_payment`.`address`,`processed_payment`.`date` FROM `clinic_database`.`processed_payment` where confirm_code="IWWF8"',(err, rows) => {
        if (err) throw err;
         console.log(rows);
